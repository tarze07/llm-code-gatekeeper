"""Testy adaptera tsc/eslint na zapisanych próbkach prawdziwego wyjścia.

Próbki w `tests/data/` pochodzą z realnych uruchomień (`tsc --noEmit --pretty
false`, `eslint --format json`) na małych fixture'ach — nie z pamięci.
Ścieżki bezwzględne są zneutralizowane do `/repo/...`, żeby fixture nie
zależał od katalogu, w którym została nagrana; reszta struktury (poziomy,
kody błędów, `message`) jest nietknięta.
"""
